...
interface ILongStrategy is ILongStrategyEvents {
    ...
    /// @param collateral - collateral to withdraw in terms of liquidity invariant units
    /// @return tokensHeld - updated collateral token amounts backing loan
    function _decreaseCollateral(uint256 tokenId, uint128[] calldata amounts, address to, uint256 collateral) external returns(uint128[] memory tokensHeld);
