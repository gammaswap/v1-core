...
interface ILongStrategy is ILongStrategyEvents {
    ...
    /// @param ratio - ratio to rebalance collateral after decreasing collateral
    /// @return tokensHeld - updated collateral token amounts backing loan
    function _decreaseCollateral(uint256 tokenId, uint128[] calldata amounts, address to, uint256[] calldata ratio) external returns(uint128[] memory tokensHeld);
