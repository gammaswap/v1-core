...
contract TestLongStrategy is ILongStrategy {
    ...
    function _borrowLiquidity(uint256 tokenId, uint256 lpTokens) external override returns(uint128[] memory amounts) {
        amounts = new uint128[](2);
        amounts[0] = uint128(lpTokens * 2);
        amounts[1] = uint128(lpTokens);
        emit LoanUpdated(tokenId, amounts, 31, 32, 33);
    }

    function _repayLiquidity(uint256 tokenId, uint256 liquidity) external override returns(uint256 liquidityPaid, uint128[] memory amounts){
        liquidityPaid = tokenId;
        amounts = new uint128[](2);
        amounts[0] = 9;
        amounts[1] = 10;
        emit LoanUpdated(tokenId, amounts, uint128(liquidity), 42, 43);
    }
