...
contract TestBorrowStrategy is BorrowStrategy {
    ...
    function balanceOf(address _address) external virtual view returns(uint256) {
    ...
    function setLPTokenBalance(uint256 lpInvariant, uint256 lpTokenBalance, uint256 lastCFMMInvariant, uint256 lastCFMMTotalSupply) public virtual {
        ...
        s.lastCFMMInvariant = uint128(lastCFMMInvariant);
        s.lastCFMMTotalSupply = lastCFMMTotalSupply;
        _mint(msg.sender, lpTokenBalance);
    }
    ...
    function testCalcOriginationFee(uint16 _origFee, uint32 emaUtilRate, uint8 minUtilRate, uint16 feeDivisor, uint256 liquidityBorrowed, uint256 borrowedInvariant, uint256 lpInvariant, uint256 discount) external virtual {
