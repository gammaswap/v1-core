...
abstract contract AbstractGammaPoolFactory is IGammaPoolFactory, TwoStepOwnable {
    ...
    function setPoolParams(address _pool, uint16 _origFee, uint8 _extSwapFee, uint8 _emaMultiplier, uint8 _minUtilRate, uint8 _maxUtilRate, uint16 _feeDivisor, uint8 _liquidationFee, uint8 _ltvThreshold) external virtual override {
        isForbidden(feeToSetter); // only feeToSetter can update origination fee parameters
        IGammaPool(_pool).setPoolParams(_origFee, _extSwapFee, _emaMultiplier, _minUtilRate, _maxUtilRate, _feeDivisor, _liquidationFee, _ltvThreshold);
        emit PoolParamsUpdate(_pool, _origFee, _extSwapFee, _emaMultiplier, _minUtilRate, _maxUtilRate, _feeDivisor, _liquidationFee, _ltvThreshold);
    }
