...
abstract contract GammaPool is IGammaPool, GammaPoolERC4626, Refunds {
    ...
    function getLatestPoolData() external virtual override view returns(PoolData memory data) {
        ...
        .getLastFees(s.factory, borrowedInvariant, data.LP_TOKEN_BALANCE, data.lastCFMMInvariant, data.lastCFMMTotalSupply,
            s.lastCFMMInvariant, s.lastCFMMTotalSupply, data.LAST_BLOCK_NUMBER, address(this));
    ...
    function decreaseCollateral(uint256 tokenId, uint128[] calldata amounts, address to, uint256 collateral) external virtual override returns(uint128[] memory tokensHeld) {
        return abi.decode(callStrategy(longStrategy, abi.encodeWithSelector(ILongStrategy._decreaseCollateral.selector, tokenId, amounts, to, collateral)), (uint128[]));
    }
